...
import { useDerived, computeDerived } from "./useDerived";
import {
  blackoutActiveAt,
 ...
 *
 * PERFORMANCE — the work is split across two memos so the live clock is cheap:
 *   - `computeDayDynamics` runs the expensive 24-hour integration (49 points ×
 *     up to 60 substeps of coupled T/humidity physics) + the plant state. It is
 *     keyed on [inputs, climate, dayOfYear, derived] — it does NOT depend on the
 *     hour, so it runs once per simulated day, not once per clock tick.
 *   - `computeSnapshot` runs a single `computeAt` at the current hour + the
 *     HUD-scope VPD/vent/dewpoint calcs. It is keyed on [day, hourOfDay, inputs]
 *     and is ~1/36 the cost of the day integration (measured, not estimated).
 * The win lands whenever the HOUR changes without the DAY: scrubbing the
 * time-of-day slider, the dawn/noon/dusk/midnight presets, the paused HUD, and
 * range-play over a single day (the default "watch the day unfold" range is one
 * summer day, hour 4→22). During fast continuous playback the day advances
 * nearly every tick (default speed 168 sim-hr/s), so the day integration is
 * paid then — but the split is never slower than the old single memo.
 */
...
/**
 * Day-keyed dynamics: the expensive 24-hour integration, the plant state, and a
 * `computeAt` closure the snapshot re-uses for the current hour. Computed once
 * per simulated day.
 *
 * LOAD-BEARING INVARIANT: `computeAt` takes `hourOfDay` as a PARAMETER and never
 * reads a clock value from scope (it closes only over inputs/climate/derived/
 * dayOfYear). That is exactly what lets the snapshot recompute per tick without
 * re-integrating the whole day. If `computeAt` is ever changed to read the
 * live hour from scope, this function would have to depend on the hour and the
 * per-tick optimisation collapses. Keep it a pure per-day closure.
 */
export function computeDayDynamics(
  ...
  const monthIdx = (() => {
    ...
    for (let i = 0; i < 12; i++) if (dayOfYear <= cum[i]) return i;
    return 11;
  })();
  ...
  const computeAt = (
    ...
    const sun = sunPositionAt(lat, dayOfYear, hourOfDay);
  ...
  // ---- Plant growth state (day-keyed) ----
  // Depends only on dayOfYear + inputs + derived (NOT the live hour), so it
  // belongs here in the day memo — recomputing it every clock tick would waste
  // the phase/geometry calc. Cycle-average DLI proxy: in a production setup
  // with sized supplemental lighting, the canopy actually sees its targetDLI
  // during flower (supplemental closes the seasonal gap). When the operator
  // screens a low-target-DLI design (target < 30), the low-DLI damping branch
  // in co2YieldMultiplier correctly fires and shows under-developed plants in
  // the live visual. ventilationMode is also passed so open-vented + CO₂
  // doesn't fake a yield bump in the live geometry.
  ...
  return { trace, monthIdx, plant, computeAt };
}

export type DayDynamics = ReturnType<typeof computeDayDynamics>;

/**
 * Hour-keyed snapshot: cheap, recomputed every clock tick from the day's
 * precomputed trace + `computeAt` closure. Everything here roots at the live
 * hour (the snapshot HUD state, VPD, dewpoint, and the richer vent decision).
 */
export function computeSnapshot(
  ...
  const { trace, plant, computeAt } = day;
  ...
  const idx = Math.max(0, Math.min(trace.length - 1, Math.round(hourOfDay / TRACE_STEP_HOURS)));
...
export function useLiveDynamics() {
  ...
  // Day-keyed: the expensive 24-hour integration + plant, recomputed only when
  // the day (or inputs/climate/derived) changes — NOT on every clock tick.
  const day = useMemo(
    () => computeDayDynamics(inputs, climate, derived, sim.dayOfYear),
    [inputs, climate, derived, sim.dayOfYear],
  );

  // Hour-keyed: a single computeAt at the current hour + HUD-scope calcs.
  const snapshot = useMemo(
    () => computeSnapshot(day, sim.hourOfDay, inputs),
    [day, sim.hourOfDay, inputs],
  );

  return { snapshot, trace: day.trace, monthIndex: day.monthIdx };
}
